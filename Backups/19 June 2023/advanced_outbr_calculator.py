...
def get_rate(date):
    i = 0
    #Check if rate are loaded first
    try:
        rates_dates[i]
    except:
        return 0

    if date<rates_dates[0]:
        return rates[0]

    if date>rates_dates[-1]:
        return rates[-1]
    ...
    while not rate_found:
        try:
            rate_index = rates_dates.index(new_date)
        except ValueError:
            new_date = new_date-1
        else:
...
def calculate_balances(type,tenure,interest_amount,principal_amount,penalty_rate,payment_dates,usd_payments,zwl_payments,interest_rate,due_dates,grace_period,in_duplum_rule):
    ...
    for t in range(len(due_dates)-1):
        ...
        if t%2 != 0:
            ...
            #Remove the penalties in this part
            #-------------------------------------------------------------------------------------------------
...
def collect_information(data, row):
    ...
    payments = list(data.iloc[[row],11:].values[0])
    penalty_rate = (data.iloc[[row],9]).values[0]
    grace_period = (data.iloc[[row],10]).values[0]
    payments_dates =(list(data.columns[11:]))
    return (tenure,interest_amount,principal_amount,interest_rate,payments_dates,payments,penalty_rate,paid_off,disbursement_date,grace_period)
...
def get_outbr_balances(data,final_date, path, product_prefix):
    ...
    # for k in range(1):
    #     row = 174
        tenure,interest_amount,principal_amount,interest_rate,all_payment_dates,payments,penalty_rate,paid_off,disbursement_date, grace_period = collect_information(data, row)
        ...
        if paid_off==1:
            ...
            l = [(data.iloc[[row],0]).values[0],(data.iloc[[row],2]).values[0]]+calculate_balances(2,tenure,interest_amount,principal_amount,penalty_rate,payment_dates,payments,zwl_payments,interest_rate,due_dates,grace_period,True)
        all.append(l)
