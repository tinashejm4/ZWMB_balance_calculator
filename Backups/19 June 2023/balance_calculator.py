...
settings_frame           = Frame(root, bg=bg_color)
income_allocator         = Frame(root, bg=bg_color)
date_choice              = IntVar(inbr_stament_maker,2)
...
only_expired             = IntVar(inbr_stament_maker,0)
#Change path here
...
og_path                  = p[0]+"\\"+p[1]+"\\"+p[2]
start_path               = og_path+"\\OneDrive - Zimbabwe Womens Microfinance Bank\\Credit\\"

def change_path(new_platform):
    global start_path
    if new_platform == "OneDrive":
        start_path = og_path + "\\{}\\".format("OneDrive - Zimbabwe Womens Microfinance Bank\\Credit")
    else:
        start_path = og_path + "\\{}\\".format("Dropbox")
...
def calculate_outbr_balance(len_values):
    messages            = []
    errors              = []
    paths               = []
    this_final_date     = 0
...
def calculate_inbr_statement():
    report   = to_inbr_statement_calculator(start_path,main_payments.get(),extra_payments.get(),in_duplum.get(),user_name,statement_type.get(), account_id.get())
    messages = report[0]
    errors   = report[1]
    paths    = report[2]
...
def opener_packer():
    ...
    user_name = Label(opener,text = "Welcome {}".format(full_name[0]),bg = bg_color, fg = fg_color, font=("Arial", 8))
    ...
    user_name.grid(column = 2, row=1,padx=4,pady=5)
    ...
    btn1.grid(row = 2, column = 1, padx=4,pady=4)
    ...
    btn2.grid(row = 2, column = 3, padx=4,pady=4)
    ...
    btn3.grid(row = 3, column = 1, padx=4,pady=4)
    ...
    btn4.grid(row = 3, column = 3, padx=4,pady=4)
    ...
    btn5.grid(row = 4, column = 1, padx=4,pady=4)
    
    Label(opener,text = "",bg = bg_color, fg = 'red').grid(row = 6, column = 1, padx=4,pady=4)
    ...
    features.grid(row = 9, column = 1, padx=4,pady=4)
    
    settings = Button(opener, text = 'Settings',width=15,fg = fg_color, bd=0, height=1,bg=mid_color,activebackground=mid_color_dark,command =lambda: switch_frame(opener,settings_frame))
    settings.grid(row = 9, column = 3, padx=4,pady=4)
...
def outbr_stament_maker_packer():
    ...
    Radiobutton(outbr_stament_maker, text = "As at today", variable = date_choice,value = 2,fg = fg_color, bg = bg_color,activebackground=mid_color).grid(column=0, row=row+2, sticky="w")
    Radiobutton(outbr_stament_maker, text = "As at {}".format(get_last_day()), variable = date_choice,value = 1,fg = fg_color, bg = bg_color,activebackground=mid_color).grid(column=2, row=row+2, sticky="w")
...
def outbr_balance_calculator_packer():
    ...
    calculate = Button(outbr_balance_calculator, text = 'Calculate',width=10,fg = fg_color, bd=0, height=2,bg=mid_color,activebackground=mid_color,command =lambda: calculate_outbr_balance(len(values)))
    calculate.grid(row = row+5, column = 2, padx=4, pady=4)
...
def inbr_stament_maker_packer():
    ...
    account_id_label = Label(inbr_stament_maker,text = "Enter Client Account Id",bg = bg_color, fg = fg_color, font=("Arial", 9))
    account_id_label.grid(column = 1, row=1, pady=5)
    
    date_entry = Entry(inbr_stament_maker,textvariable = account_id, font=('calibre',10,'normal'))
    ...
    payments.grid(column = 1, row=3, pady=5)
    
    Checkbutton(inbr_stament_maker, text="Main Payments", variable=main_payments,bg = bg_color, fg = fg_color,activebackground=mid_color,font=("Arial", 10)).grid(row=4, column = 0)
    Checkbutton(inbr_stament_maker, text="Extra Payments", variable=extra_payments,bg = bg_color, fg = fg_color,activebackground=mid_color,font=("Arial", 10)).grid(row=4, column = 2)
    ...
    home_btn.grid(row     = 9, column = 0, padx=4, pady=4)
    ...
    colculate.grid(row     = 9, column = 2, padx=4, pady=4)
...
def income_allocator_packer():
    ...
    chosen_month.set(months[current_month-2])
    ...
    month_drop.grid(row = 2, column = 2, pady=2)
    
    month_drop["menu"].config(bg = mid_color,fg = fg_color)
    
    Label(income_allocator,text = "Choose Year",bg = bg_color, fg = fg_color, font=("Arial", 9)).grid(column = 0, row=3,pady=5)
    ...
    year_drop.grid(row = 3, column = 2, pady=2)
    ...
    currency_drop.grid(row = 5, column = 2, pady=2)
...
def settings_packer():
    ...
    platforms = ["OneDrive","Dropbox"]
    chosen_platform = StringVar()
    chosen_platform.set(platforms[0])
    Label(settings_frame,text = "Choose Main Folder",bg = bg_color, fg = fg_color, font=("Arial", 10)).grid(column = 0, row=1,pady=5)
    
    month_drop = OptionMenu(settings_frame , chosen_platform , *platforms, command=change_path)
    month_drop.config(width = 12, bg = bg_color, fg = fg_color,activebackground=mid_color)
    month_drop.grid(row = 1, column = 3)
    
    back_btn = Button(settings_frame, text = 'Back',width=10,fg = fg_color, bd=0, height=2,bg=mid_color,activebackground=mid_color,command = lambda: switch_frame(settings_frame,opener))
    back_btn.grid(row = 10, column = 1, padx=4, pady=4)
