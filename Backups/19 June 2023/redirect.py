...
def to_out_br_statement(final_date, file_path,save_path,path,frequency,product_id,start_path,product_prefix):
    ...
    tenure,interest_amount,principal_amount,interest_rate,all_payment_dates,payments,penalty_rate,paid_off,disbursement_date,grace_period = get_data_1(data,loan_index)
...
def unbundle_data(data,index,currency):
    ...
    if currency == 1:
        usd_loan_amount    = (data.iloc[[index],16]).values[0]
        usd_instalment     = (data.iloc[[index],17]).values[0]
        instalment         = usd_instalment
        loan_amount        = usd_loan_amount
        principal_amount   = usd_loan_amount/(tenure-grace_period)
...
def get_inbr_balances(source_data, payments_data, final_date, in_duplum , loan_book_format, only_expired,currency):
    all = []
    ...
    #     row = 1527
        ...
        intrest_rate,penalty_rate,disbursement_date,grace_period,principal_amount,interest_amount) = unbundle_data(source_data,row,currency)
...
def to_inbr_balance_calculator(start_path,final_date,user_name,in_duplum,loan_book_format,currency,only_expired):
    ...
    if currency==0:
        reset_rates()
        source_path     = get_personal_path(start_path+"Payoffs\\Statements\\",user_name)+"a_inputs\\writeoff_list.csv"
    elif currency == 1:
        load_rates(start_path)
        source_path     = get_personal_path(start_path+"Payoffs\\Statements\\",user_name)+"a_inputs\\indexing_list.csv"
    for year in range(2019,to_year+1):
    ...
    return save_data(get_inbr_balances(source_data,payments_data,final_date, in_duplum, loan_book_format, only_expired,currency),save_path,'Loans Output')
